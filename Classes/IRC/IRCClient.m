		...
		if (NSObjectIsEmpty(anick)) {
			...
			if (type == LINE_TYPE_NOTICE) {
					if ([text hasPrefix:@"*** Notice -- Client connecting"] || 
						...
						// this prints it to status which we don't want
						// [self printBoth:nil type:type text:text];	
						
                        
                        // Making other information not being processed for whatever reason also show in channel
                        // lets also make this only show if we want them :)
                        if ([Preferences handleServerNotices]) {
                            
                        // let's deprecate sending to the "Server Notices" tab
                        ...
                        //else lets just print to status
                        else {
                            [self printBoth:nil type:type text:text];
                        }
						BOOL processData = YES;
						...
						if ([Preferences handleServerNotices]) {
							// discontinue this code as the preferences box has been removed
